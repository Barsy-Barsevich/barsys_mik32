...
void HAL_TSENS_Init()
{
    ...
    HAL_TSENS_ClockDivider(499); //< Делитель 500
    HAL_TSENS_SetTrim(128); //< Настройка Trim = 128
}
...
bool HAL_TSENS_ClockSource(uint8_t data)
{
    ...
    ANALOG_REG->TSENS_CFG &= 0b11111111111111111100011111111111;
    ANALOG_REG->TSENS_CFG |= (_hal_tsens_clkmux_ << 11);
    return true;
}
...
bool HAL_TSENS_Clock(uint32_t f_enter)
{
    ...
    float up_border = f_real / 100000UL;
    float down_border = f_real / 32000UL;
...
void HAL_TSENS_SetTrim(uint8_t data)
{
    ...
    ANALOG_REG->TSENS_CFG |= (data << TSENS_CFG_TRIM_S);
}
...
void HAL_TSENS_SetLowThreshold(uint16_t data)
{
    ...
    ANALOG_REG->TSENS_THRESHOLD |= ((uint32_t)data << 10);
}
...
void HAL_TSENS_SetHiThreshold(uint16_t data)
{
    ...
    ANALOG_REG->TSENS_THRESHOLD |= (uint32_t)data;
}
...
float HAL_TSENS_ReadMeasurement()
{
    ...
    return (((float)value * 619.2F) / 1024.0F) - 273.15F;
}

float HAL_TSENS_SingleMeasurement()
{
    ANALOG_REG->TSENS_SINGLE = 1;
    //while (!(ANALOG_REG->TSENS_IRQ & (1<<TSENS_IRQ_EOS_S)));
